...
def predict_crop():
    try:
        ...
        with ThreadPoolExecutor() as executor:
            temp_future = executor.submit(predict_feature, temp_df, month, year)
            humi_future = executor.submit(predict_feature, humi_df, month, year)
            rain_future = executor.submit(predict_feature, rain_df, month, year)

            temperature = temp_future.result()
            humidity = humi_future.result()
            rainfall = rain_future.result()
